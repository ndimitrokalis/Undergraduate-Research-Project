...
import { getMediaByType, getWatchHistory, getFavorites, searchMedia } from '../api/api';
...
export default function Browse({ type }) {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([getMediaByType(type), getWatchHistory(), getFavorites()])
      .then(([mediaData, historyData, favData]) => {
        setAllMedia(mediaData);
        setDisplayMedia(mediaData);
        setContinueWatching(historyData.filter((h) => !h.completed && h.progressSeconds > 0 && h.mediaType === type));
        setWatchedIds(new Set(historyData.filter((h) => h.completed && h.mediaType === type).map((h) => h.mediaId)));
        const pm = new Map();
        historyData.filter((h) => h.mediaType === type).forEach((h) => {
  ...
  function handleSearchResults(results, term) {
    setDisplayMedia(results.filter((m) => m.type === type));
    setSearchTerm(term);
  }
    ...
    <div className="min-h-screen bg-[#141414]">
      ...
      <main className="px-8 py-8">
        {searchTerm ? (
          ...
          <p className="text-[#737373] text-base">No {label.toLowerCase()} available yet.</p>
        ) : (
          <>
            <h1 className="text-white text-[28px] font-bold mb-6">{label}</h1>
