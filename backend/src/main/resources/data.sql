...
-- Demo users (password: Testing1!)
INSERT INTO users (email, password, display_name, full_name, phone, country, enabled, admin, subscription_type, created_at)
SELECT 'alice@example.com', '$2b$12$F7bfGTBum/nQlY76kilj3OTNl7BuaO.LuJBAeHePuvtI/5OJH/7eq', 'Alice', 'Alice Johnson', '+30 6911111111', 'Greece', true, false, 'FREE', NOW()
WHERE NOT EXISTS (SELECT 1 FROM users WHERE email = 'alice@example.com');

INSERT INTO users (email, password, display_name, full_name, phone, country, enabled, admin, subscription_type, created_at)
SELECT 'bob@example.com', '$2b$12$F7bfGTBum/nQlY76kilj3OTNl7BuaO.LuJBAeHePuvtI/5OJH/7eq', 'Bob', 'Bob Smith', '+44 7911222333', 'UK', true, false, 'FREE', NOW()
WHERE NOT EXISTS (SELECT 1 FROM users WHERE email = 'bob@example.com');

INSERT INTO users (email, password, display_name, full_name, phone, country, enabled, admin, subscription_type, created_at)
SELECT 'admin@example.com', '$2b$12$F7bfGTBum/nQlY76kilj3OTNl7BuaO.LuJBAeHePuvtI/5OJH/7eq', 'Admin', 'Admin User', '+1 5551234567', 'US', true, true, 'FREE', NOW()
